...
int rx_app(int mode, int packet_cnt) {
    ...
    memcpy(rx_arg.devname, DEF_RX_DEVICE_NAME, sizeof(DEF_RX_DEVICE_NAME));
    rx_arg.mode = mode;
    rx_arg.size = packet_cnt;
    pthread_create(&tid1, NULL, rx_thread, (void *)&rx_arg);
#ifdef _MULTI_CPU_CORES_
    cpu_set_t cpuset1;
    CPU_ZERO(&cpuset1);
    CPU_SET(1, &cpuset1);
...
int process_main_rxCmd(int argc, const char *argv[], menu_command_t *menu_tbl);
...
menu_command_t  mainCommand_tbl[] = {
    ...
    { "rx",    EXECUTION_ATTR,   process_main_rxCmd, \
        "   rx -m <mode> -c <count>", \
        "   Run rx test application with data szie in mode\n"
        "            <mode> default value: 0 (0: tsn, 1: normal, 2: loopback-integrity check, 3: performance)\n"
        "            <count> default value: 1 (1 ~ 255)"},
    ...
    { "ioctl", EXECUTION_ATTR,   process_main_ioctlCmd, \
        "   ioctl [thread, get, set]", \
        "   Retrieve or set device information using ioctl.\n"
        "         thread : Manuplate the thread using ioctl command\n"
        "            get : Retrieve information about the device using ioctl\n"
        "            set : Change information about the device using ioctl"},
...
int process_ioctl_getCmd(int argc, const char ** argv, menu_command_t *menu_tbl);
int process_ioctl_threadCmd(int argc, const char ** argv, menu_command_t *menu_tbl);
menu_command_t  ioctlCommand_tbl[] = {
    { "get",    EXECUTION_ATTR,   process_ioctl_getCmd, \
        "   ioctl get [ maxBuf, engine ] < -d device >", \
        "   Retrieve information about the device using ioctl\n"
        "            maxBuf : maximum size of a single DMA transfer descriptor\n"
        "            "},
    { "thread", EXECUTION_ATTR,   process_ioctl_threadCmd, \
        "   ioctl thread [ init, exit, start, stop ] < -d device >", \
        "   Start or stop a thread using ioctl\n"
        "             init : Performs initialization work to create a thread\n"
        "             exit : Delete  the thread and remove related data\n"
...
int32_t fn_ioctl_get_maxBufArgument(int32_t argc, const char *argv[]);
int32_t fn_ioctl_get_engineArgument(int32_t argc, const char *argv[]);
argument_list_t  ioctlGetArgument_tbl[] = {
        {"maxBuf",  fn_ioctl_get_maxBufArgument},
        {"engine",  fn_ioctl_get_engineArgument},
        {0,     NULL}
    };

int32_t fn_ioctl_thread_initArgument(int32_t argc, const char *argv[]);
int32_t fn_ioctl_thread_exitArgument(int32_t argc, const char *argv[]);
int32_t fn_ioctl_thread_startArgument(int32_t argc, const char *argv[]);
int32_t fn_ioctl_thread_stopArgument(int32_t argc, const char *argv[]);
argument_list_t  ioctlThreadArgument_tbl[] = {
        {"init",  fn_ioctl_thread_initArgument},
        {"exit",  fn_ioctl_thread_exitArgument},
        {"start", fn_ioctl_thread_startArgument},
        {"stop",  fn_ioctl_thread_stopArgument},
        {0,     NULL}
    };

int process_main_ioctlCmd(int argc, const char *argv[],
                                     menu_command_t *menu_tbl) {
    int  id;
    ...
    return lookup_cmd_tbl(argc, (const char **)pav, ioctlCommand_tbl, ECHO);
}

#define MAIN_IOCTL_MAX_BUF_OPTION_STRING  "d:hv"
int32_t fn_ioctl_get_maxBufArgument(int32_t argc, const char *argv[]) {

    int max_buf_size = 5;
    char devname[256] = "/dev/xdma0_c2h_0";
    int argflag;
        int rc;

    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_MAX_BUF_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
    ...
    //rc = xdma_api_ioctl_get_max_buf_length(devname, &max_buf_size);
    rc = xdma_api_ioctl_bd_insert(devname, &max_buf_size);
    if (rc == 0) {
        printf("\nIOCTL_XDMA_MAX_BUF_LEN_GET succesful(max_buf_size: %d).\n", max_buf_size);
    } else {
        printf("ioctl(..., IOCTL_XDMA_MAX_BUF_LEN_GET) = %d\n", rc);
    }
        return rc;
}

#define MAIN_IOCTL_ENGINE_OPTION_STRING  "d:hv"
int32_t fn_ioctl_get_engineArgument(int32_t argc, const char *argv[]) {
    ...
    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_ENGINE_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
            memset(devname, 0, 256);
            strcpy(devname, optarg);
    ...
    rc = xdma_api_ioctl_print_engine(devname, &version);
    if (rc == 0) {
        printf("IOCTL_XDMA_PRN_ENGINE_INFO succesful(version: %d).\n", version);
    } else {
        printf("ioctl(..., IOCTL_XDMA_PRN_ENGINE_INFO) = %d\n", rc);
    }
...
int process_ioctl_getCmd(int argc, const char *argv[], menu_command_t *menu_tbl) {
    ...
    argc--, argv++;
    for (int index = 0; ioctlGetArgument_tbl[index].name; index++) {
        if (!strcmp(argv[0], ioctlGetArgument_tbl[index].name)) {
            if(ioctlGetArgument_tbl[index].fP(argc, argv)) {
                print_argumentWarningMessage(pargc, (const char **)pav, menu_tbl, NO_ECHO);
            }
            return 0;
        }
    }
...
#define MAIN_IOCTL_THR_INIT_OPTION_STRING  "d:hv"
int BufferCount = ENGINE_NUMBER_OF_BUFFER;
int32_t fn_ioctl_thread_initArgument(int32_t argc, const char *argv[]) {
    ...
    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_THR_INIT_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
            memset(devname, 0, 256);
            strcpy(devname, optarg);
    ...
    rc = xdma_api_ioctl_thread_init(devname, BufferCount);
    if (rc == 0) {
        printf("IOCTL_XDMA_THREAD_INIT succesful(BufferCount: %d).\n", BufferCount);
    } else {
        printf("ioctl(..., IOCTL_XDMA_THREAD_INIT) = %d\n", rc);
    }

    return rc;
}

#define MAIN_IOCTL_THR_EXIT_OPTION_STRING  "d:hv"
int32_t fn_ioctl_thread_exitArgument(int32_t argc, const char *argv[]) {
    ...
    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_THR_EXIT_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
            memset(devname, 0, 256);
            strcpy(devname, optarg);
    ...
    rc = xdma_api_ioctl_thread_exit(devname, &version);
    if (rc == 0) {
        printf("IOCTL_XDMA_THREAD_EXIT succesful(version: %d).\n", version);
    } else {
        printf("ioctl(..., IOCTL_XDMA_THREAD_EXIT) = %d\n", rc);
    }

    return rc;
}

#define MAIN_IOCTL_THR_START_OPTION_STRING  "d:hv"
int32_t fn_ioctl_thread_startArgument(int32_t argc, const char *argv[]) {
    ...
    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_THR_START_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
            memset(devname, 0, 256);
            strcpy(devname, optarg);
    ...
    rc = xdma_api_ioctl_thread_start(devname, &version);
    if (rc == 0) {
        printf("IOCTL_XDMA_THREAD_START succesful(version: %d).\n", version);
    } else {
        printf("ioctl(..., IOCTL_XDMA_THREAD_START) = %d\n", rc);
    }

    return rc;
}

#define MAIN_IOCTL_THR_STOP_OPTION_STRING  "d:hv"
int32_t fn_ioctl_thread_stopArgument(int32_t argc, const char *argv[]) {
    ...
    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_IOCTL_THR_STOP_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'd':
            memset(devname, 0, 256);
            strcpy(devname, optarg);
    ...
    rc = xdma_api_ioctl_thread_stop(devname, &version);
    if (rc == 0) {
        printf("IOCTL_XDMA_THREAD_STOP succesful(version: %d).\n", version);
    } else {
        printf("ioctl(..., IOCTL_XDMA_THREAD_STOP) = %d\n", rc);
    }
...
int process_ioctl_threadCmd(int argc, const char *argv[], menu_command_t *menu_tbl) {
    ...
    argc--, argv++;
    for (int index = 0; ioctlThreadArgument_tbl[index].name; index++) {
        if (!strcmp(argv[0], ioctlThreadArgument_tbl[index].name)) {
            if(ioctlThreadArgument_tbl[index].fP(argc, argv)) {
                print_argumentWarningMessage(pargc, (const char **)pav, menu_tbl, NO_ECHO);
            }
            return 0;
        }
...
#define MAIN_RX_OPTION_STRING  "m:c:hv"
int process_main_rxCmd(int argc, const char *argv[],
                            menu_command_t *menu_tbl) {
    int mode  = RUN_MODE_NORMAL;
    int packet_cnt = 1;
    int argflag;

    while ((argflag = getopt(argc, (char **)argv,
                             MAIN_RX_OPTION_STRING)) != -1) {
        switch (argflag) {
        case 'm':
            if (str2int(optarg, &mode) != 0) {
                printf("Invalid parameter given or out of range for '-m'.");
            ...
            break;
        case 's':
            if (str2int(optarg, &packet_cnt) != 0) {
                printf("Invalid parameter given or out of range for '-s'.");
                return -1;
            }
            if ((packet_cnt < 64) || (packet_cnt > MAX_BUFFER_LENGTH)) {
                printf("packet_cnt %d is out of range.", packet_cnt);
                return -1;
            }
            break;
        case 'v':
