...
u64 alinx_get_tx_drop_packets(struct pci_dev *pdev) {
	...
	if (regval != 0) {
		pr_err("Packet dropped: tx_drop_packets\n");
	}
...
u64 alinx_get_normal_timeout_packets(struct pci_dev *pdev) {
	...
	if (cnt != regval) {
		pr_err("Packet dropped: normal_timeout\n");
	}
	cnt = regval;
...
u64 alinx_get_to_overflow_popped_packets(struct pci_dev *pdev) {
	...
	if (cnt != regval) {
		pr_err("Packet dropped: to_overflow_dropped\n");
	}
	cnt = regval;
...
u64 alinx_get_to_overflow_timeout_packets(struct pci_dev *pdev) {
	...
	if (cnt != regval) {
		pr_err("Packet dropped: to_overflow_timeout\n");
	}
	cnt = regval;
